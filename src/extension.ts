...
export function activate(context: vscode.ExtensionContext) {
	...
	let recreatePythonEnvCommand = vscode.commands.registerCommand('arpirobot.recreatePythonEnv', recreatePythonEnv);
	context.subscriptions.push(recreatePythonEnvCommand);
...
export async function recreatePythonEnv(){
	if(vscode.workspace && vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0){
		createPythonEnvironment(vscode.workspace.workspaceFolders[0], true)	
	}
}
